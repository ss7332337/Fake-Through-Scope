...
float4 main(float4 vpos : SV_Position, float2 texcoord : TEXCOORD0) : SV_Target 
{
	...
	//float2 texcoordCorrected = texcoord;
	//float2 texcoordCorrected = aspect_ratio_correction(texcoord) ;

	float2 corrected_texturecoords = aspect_ratio_correction(texcoord);
	float4 color = tBACKBUFFER.Sample(gSamLinear, corrected_texturecoords);
	...
	// if (abseyeTranslationLerp.y < 0 && abseyeTranslationLerp.y >= -0.001)
	// 	abseyeTranslationLerp.y = -0.001;
	// else if (abseyeTranslationLerp.y >= 0 && abseyeTranslationLerp.y <= 0.001)
	// 	abseyeTranslationLerp.y = 0.001;
	...
	color.rgb -= float3(100,100,100) *  step(2,distToCenter);
