...
float4 main(float4 vpos : SV_Position, float2 texcoord : TEXCOORD0) : SV_Target 
{
	...
	//float2 texcoordCorrected;

	// texcoordCorrected = texcoord - float2(0.5f,0.5f);
	// texcoordCorrected *= rcp(10.0F);
	// texcoordCorrected += float2(0.5f,0.5f);
	...
	// if (abseyeDirectionLerp.y < 0 && abseyeDirectionLerp.y >= -0.0025)
	// 	abseyeDirectionLerp.y = -0.0025;
	// else if (abseyeDirectionLerp.y >= 0 && abseyeDirectionLerp.y <= 0.0025)
	// 	abseyeDirectionLerp.y = 0.0025;
